...
--   4. dining_areas
--   5. tables
--   6. roles (bootstrap seeder đã tạo, dùng INSERT IGNORE để an toàn)
...
VALUES
  ...
  ('ML',    'Mililit',     'mL',    'ML',    'mL',    'Đơn vị thể tích nhỏ',     NOW(), NOW()),
...
--   phone    = 0901000004
-- BCrypt hash của "GoldenHeart@2026" (đã verify với Spring BCryptPasswordEncoder cost=12)
...
-- 14.1 CUSTOMER TIERS (cấu hình loyalty)
INSERT IGNORE INTO customer_tiers
  (id, code, name, min_points, discount_rate, active, note, created_at, updated_at)
VALUES
  (1, 'BRONZE',   'Bronze',   100, 1.00, TRUE, 'Hạng loyalty mặc định', NOW(), NOW()),
  (2, 'SILVER',   'Silver',   200, 2.00, TRUE, 'Hạng loyalty mặc định', NOW(), NOW()),
  (3, 'GOLD',     'Gold',     500, 3.00, TRUE, 'Hạng loyalty mặc định', NOW(), NOW()),
  (4, 'PLATINUM', 'Platinum', 800, 4.00, TRUE, 'Hạng loyalty mặc định', NOW(), NOW()),
  (5, 'DIAMOND',  'Diamond', 1000, 5.00, TRUE, 'Hạng loyalty mặc định', NOW(), NOW());

-- 15. WORK SHIFTS
-- Module operations hiện đã có entity, có thể bổ sung thêm seed sau nếu cần

-- =============================================================================
-- 16. DỮ LIỆU NGHIỆP VỤ DÙNG CHO POSTMAN E2E
-- =============================================================================

-- 16.1 Trạng thái edge-case cho menu
UPDATE menu_items
...
-- 16.3 Orders bao phủ đủ trạng thái: đang xử lý, hoàn tất, đã thanh toán, đã hủy, thanh toán một phần
...
-- 16.4 Order items với trạng thái bếp thực tế, ghi chú rõ ràng và lý do hủy
...
-- 16.5 Bills, payments và loyalty traces cho luồng revenue và checkout
...
VALUES
  (1, 1, NULL, 'ADJUSTMENT_IN',   80,    0,   80,  'Thiết lập điểm loyalty ban đầu cho dữ liệu demo',                '2026-04-10 09:00:00'),
  (2, 2, NULL, 'ADJUSTMENT_IN',  150,    0,  150,  'Thiết lập điểm loyalty ban đầu cho dữ liệu demo',                '2026-04-11 18:30:00'),
  (3, 3, NULL, 'ADJUSTMENT_IN',  210,    0,  210,  'Thiết lập điểm loyalty ban đầu cho dữ liệu demo',                '2026-04-12 12:15:00'),
  (4, 4, NULL, 'ADJUSTMENT_IN',  520,    0,  520,  'Thiết lập điểm loyalty trước bill seed đã thanh toán',           '2026-04-15 17:00:00'),
  (5, 4,    1, 'EARN',            22,  520,  542,  'Điểm nhận được từ bill seed #1 đã thanh toán',                  '2026-04-16 19:10:00'),
  (6, 5, NULL, 'ADJUSTMENT_IN', 1005,    0, 1005,  'Thiết lập điểm loyalty ban đầu cho khách hạng Diamond demo',    '2026-04-13 20:45:00');

-- 16.6 Stock movements lịch sử dùng cho gross profit và luồng bếp
...
VALUES
  (1, 1, 11, 1, 1, 7, 'SALE_OUT', -0.20, 39.80,  18000.00,  3600.00, '2026-04-17 10:05:00', 'Trừ kho khi order item seed #1 chuyển sang PROCESSING'),
  (2, 1,  1, 1, 1, 7, 'SALE_OUT', -0.10, 49.90, 280000.00, 28000.00, '2026-04-17 10:05:00', 'Trừ kho khi order item seed #1 chuyển sang PROCESSING'),
  (3, 1, 12, 1, 1, 7, 'SALE_OUT', -0.15, 59.85,  50000.00,  7500.00, '2026-04-17 10:05:00', 'Trừ kho khi order item seed #1 chuyển sang PROCESSING'),
...
-- 16.7 Inventory action logs dùng để test audit trail
...
VALUES
  ...
  (2, 13, 1, 'Chi nhánh Quận 1', 13, 'Hải sản hỗn hợp',   'kg',
   ...
   'kg', 'kg',
   'Điều chỉnh tồn kho seed để tạo kịch bản WAITING_STOCK có thể test lại ổn định', '2026-04-17 09:00:00'),
  (3, 11, 1, 'Chi nhánh Quận 1', 11, 'Phở bò - bánh phở', 'kg',
   ...
   'kg', 'kg',
   'Order item seed #1 tiêu thụ 0.2kg bánh phở khi bắt đầu chế biến', '2026-04-17 10:05:00');

-- 16.8 Trạng thái bàn đồng bộ với các kịch bản vận hành seed sẵn
UPDATE tables
SET status = CASE id
...
-- =============================================================================
-- TÓM TẮT ID SẠCH CHO POSTMAN
-- Roles:           ADMIN=1, MANAGER=2, STAFF=3, KITCHEN=4, CUSTOMER=5
-- Branches:        Q1=1, Q7=2, BinhThanh=3
-- Areas:           Q1Indoor=1, Q1Outdoor=2, Q7Floor1=3, Q7Floor2=4, BTMain=5
-- Tables Q1:       available=1, processing=2, billable=3, cleaning=4, reserved=5
-- Tables Q7:       available=6, partial_bill=7, large=8
-- Users:           admin=1, manager_q1=2, manager_q7=3, staff_q1_a=4,
--                  staff_q1_b=5, staff_q7_a=6, kitchen_q1=7, kitchen_q7=8
-- CustomerTiers:   Bronze=1, Silver=2, Gold=3, Platinum=4, Diamond=5
-- Categories:      Main=1, Appetizer=2, Dessert=3, Drink=4, Rice=5, Seafood=6
-- MenuItems:       Q1=1-10, Q7=11-15, BinhThanh=16-17
-- Customers:       base=1, bronze=2, silver=3, gold=4, diamond=5
...
-- Report fixtures: payment days=2026-04-16, 2026-04-17; methods=CASH, CREDIT_CARD, E_WALLET, BANK_TRANSFER
-- LoyaltyTx:       opening=1-4, bill1_earn=5, diamond_opening=6
